...
class Config:
    ...
    # Счётчик количества в строке склянки: (x, dy от центра строки, w, h).
    # Игра ставит туда столько склянок, сколько нужно до полной энергии, —
    # один тап «Использовать» может потратить не одну.
    flask_qty_region_rel: tuple = (520, -30, 100, 60)
    # Учтённый остаток склянок на старте (в окне энергии «В наличии: N»
    # перекрыт счётчиком и не читается, поэтому ведём локально; задаётся в GUI).
    # 0 = «неизвестно» -> склянки НЕ тратятся вовсе: расходник невосстановим,
    # а соблюсти flask_stop_threshold вслепую невозможно.
