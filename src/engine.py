...
class BotEngine:
    ...
    def start(self):
        if self.cfg.dry_run:
            ...
            # Тот же расклад, что у «Элитной скверны»: окно энергии видно
            # только с превью отправки, поэтому остаток на старте неизвестен.
            self.flasks = None
            self.log("Старт (присоединение к чужим штурмам). Остаток склянок "
                     "прочитаю из игры после первого рефилла.")
